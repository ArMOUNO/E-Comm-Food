...
export const CartDrawer = () => {
    ...
    const handleSelectProduct = (data, isChecked) => {
        setSelectProduct((prevProducts) => {
            if (isChecked === "false") {
                ...
                return prevProducts.filter((product) => product.idMeal !== data.idMeal);
            }
        });
    };
    ...
    return (
        <Drawer open={isOpen} onClose={closeDrawer} showCloseIcon={false}>
            ...
            <DrawerContent className='' position="right">
                <div className="flex w-full bg-white  justify-end">

                    <button onClick={closeDrawer}><IoMdClose /></button>
                </div>
                <p className='text-32l font-bold text-gray-600 border-b-2 py-2'>Shopping Cart</p>
                <div className='max-h-[600px] overflow-y-auto mt-4 bg-slate-50 py-1 rounded-t-sm'>
                    {
                                ...
                                <div key={item?.idMeal} className="overflow-y-auto">
                                    <div>
                                        <fieldset className="flex items-center gap-2">
                                            <Checkbox className="" onClick={(e) => handleSelectProduct(item, e.target.ariaChecked)} defaultChecked={false} id={item?.idMeal} />
                                            <Label htmlFor={item?.idMeal}>Select Product</Label>
                                        </fieldset>
                                    </div>
                                    <ShowCartData
                                        price={item?.idMeal}
                        ...
                        <div>
                            <div className='border-t-2 mt-3 flex justify-between'>
                                <div className='flex items-center space-x-3'>
                                    <p className='text-xl font-serif text-green-700'>SubTotal</p>
                                    <p className='text-red-700 font-bold'>Total Items: {totalOrder}</p>
                                </div>
                                <p className='text-green-700 font-bold flex items-center'>
                                    Total Price:
                                    <span className='text-red-700 flex items-center mx-2'>
                                        <FaSackDollar className='text-yellow-900 mx-1' />
                                        {totalPrice}
                                    </span>
                                </p>
                            </div>
